...
2. `docs/` のこの書類、または `docs/チャンネル分析AI_運用手順.md` を直す
...
**この書類は 2026-08-10 に Dropbox からリポジトリの `docs/` へ移しました。**
理由は、Dropbox にはセッションから書き込めない場合があり、その結果「担当者に md を更新させる」
ことになってしまうためです。`docs/` なら push でどのセッションからでも直せます。
Dropbox「PGコンソール」に残っている同名ファイルは**更新停止**です。参照しないでください。

**代わりに、この書類は公開されます。** 値そのものは本文に書かず、どこを見れば分かるかだけを書いてください。
メールアドレス、LINEグループID、Apps Script の URL、個人のフルネーム、ローカルの実パス、
社外の個人名・企業名、金額、人事、未公表のブランド変更は書かない。**迷ったら書かない。**
どのPC・どのアカウントでも最初に読む入り口は、リポジトリ直下の `CLAUDE.md` です。
...
- `docs/チャンネル分析AI_運用手順.md` — **週次AI分析の正本（分析者としての大原則・数字の落とし穴・出力形式）**

### Dropbox「PGコンソール」直下（参照のみ・md は更新停止）
...
- `CLAUDE.md` — どのPC・どのアカウントでも最初に読む作業ルール
- `変更履歴.md` — 変更と、この書類への反映状況

### ⚠ 正本はここだけ

Dropbox「PGコンソール」と別フォルダ（`ヘッドホンポーカーアーカイブ/コンソールサイト/引き継ぎ資料.md`）にも
同じサイトの引き継ぎがありますが、**正本は `docs/` のこの書類です。**
古いものを見つけたら削除するか、冒頭に「参照のみ・正本はリポジトリの docs/」と書き足してください。
