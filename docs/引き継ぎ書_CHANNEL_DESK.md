...
| # | やること | 状態・条件 |
| :-- | :---- | :---- |
| 1 | Dropbox「PGコンソール」の古い md 2つ（`引き継ぎ書_CHANNEL_DESK.md` / `チャンネル分析AI_運用手順.md`）の冒頭に「参照のみ・正本はリポジトリの `docs/`」と書き足す | **「あなたのコンピューター」で実行したタスクでないと書き込めません。** クラウド実行のセッションではデバイスブリッジのファイル操作ツールが来ないことがある |
