...
**ただし Dropbox「PGコンソール」直下の同名 md も更新し続けてください（運営者の希望）。**
`docs/` を正本として編集し、**Dropbox 側はそこからの上書きコピーで同期**します。
Dropbox 側を単独で編集しないこと（両方を別々に直すとズレます）。
デバイスブリッジが使えないセッションでは、同期を次のセッションに残してよい。
**担当者に手作業を頼まないこと。** 詳細はリポジトリ直下の `CLAUDE.md` の★の章。
...
## いま進行中・次にやること（2026-08-10 時点）

**新しいチャットはここを最初に見てください。** 済んだ項目はこの章から消し、`変更履歴.md` に移してください。

| # | やること | 状態・条件 |
| :-- | :---- | :---- |
| 1 | Dropbox「PGコンソール」直下の md 2つを `docs/` から上書きコピーして同期する（2026-08-10 時点で未同期） | **「あなたのコンピューター」で実行したタスクでないと書き込めません。** クラウド実行のセッションではデバイスブリッジのファイル操作ツールが来ないことがある。届かない場合は次のセッションに残してよい |
| 2 | 週次AI分析の起動タイミングを確定させる | この書類の旧版は「毎週土曜朝 cron `0 8 * * 6`」、運用手順 §0 は「毎週火曜 9:00 のリマインダー」。**実物のスケジュールを見て両方を揃える** |
| 3 | 運営ログ（`ops.json`）を最新にする | `ops.json` は 2026-08-07 のまま。8/10 の議事録ぶんが未反映。スプレッドシート「PG運営ログ（非公開）」に行を足したうえで「運営ログ反映して」と依頼してもらう |
| 4 | クーポンシートに「コード」列を追加してもらう | 発行元（社外の方）への依頼。集計単位を土〜金に揃える依頼も併せて。現状は動画への帰属ができない（§10） |
| 5 | 動画アーカイブの入力規則 | 入力規則が289行目までしか無い（1000行に広げるか要判断）／`E2:E6` に「国内切り抜き」が無い／企画タイプ空欄が約69行 |
| 6 | 運営ログの3時間自動反映 | **保留中の宿題。**着手条件と2案は §10 に全部書いてある |

次の週次レポートの対象は **8/8〜8/14 週**で、確定するのは 8/17 ごろです。
直前の 8/1〜8/7 週は確定版を書き終えています（`09c52b5`）。
